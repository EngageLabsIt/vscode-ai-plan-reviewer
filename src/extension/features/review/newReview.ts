...
export function registerNewReviewCommand(
  ...
  return vscode.commands.registerCommand('planReviewer.newReview', async () => {
    ...
    if (inReviewPlans.length > 0) {
      const items: ReviewPickItem[] = inReviewPlans.map(
        (plan): ExistingPlanItem => {
      ...
      const selected = await vscode.window.showQuickPick<ReviewPickItem>(
        items,
        {
