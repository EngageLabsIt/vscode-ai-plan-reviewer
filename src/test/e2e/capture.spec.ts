...
test.beforeEach(async ({ page }) => {
  ...
  await page.waitForSelector('.plan-viewer .comment-card-wrap', {
    timeout: 5_000,
  });
});
...
test('comment-card @capture', async ({ page }) => {
  test.slow();
  await page
    .locator('.plan-viewer .comment-card-wrap')
    .first()
...
test('flow-add-comment @capture', async ({ page }) => {
  ...
  await page
    .locator('.comment-form-inline textarea')
    .pressSequentially(
  ...
  await expect(page.locator('.plan-viewer .comment-card-wrap')).toHaveCount(
    initialCount + 1,
  );
...
test('flow-search @capture', async ({ page }) => {
  ...
  await page
    .locator('.search-bar__input')
    .pressSequentially('JWT', { delay: 80 });
  await page.waitForSelector('.line-row--search-match', { timeout: 3_000 });
...
test('flow-navigator @capture', async ({ page }) => {
  ...
  const navItem = page
    .locator('.comment-navigator__section-comment-item')
    .first();
