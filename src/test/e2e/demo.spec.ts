...
test('submitting a comment from inline form creates a comment card', async ({ page }) => {
...
test('opening navigator shows comment list without filter chips', async ({ page }) => {
...
test.describe('Search Feature Tests', () => {
  ...
  test('Enter moves to next search match', async ({ page }) => {
    ...
    const firstMatch = await page.locator('.line-row--search-current').getAttribute('id');

    await page.locator('.search-bar__input').press('Enter');
    const secondMatch = await page.locator('.line-row--search-current').getAttribute('id');
...
test.describe('Inline Comment Form', () => {
  ...
  test('text selection highlights rows with .line-row--selecting', async ({ page }) => {
